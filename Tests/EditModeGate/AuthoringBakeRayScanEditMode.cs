...
{
    /// <summary>
    /// EditMode port of <see cref="Zori.Entities.Physics2D.Tests.AuthoringBakeRayScanGate"/>: the FAITHFUL
    /// authoring→bake confirmation of the 360-ray collider-parity scan. For each shape kind and each of three
    /// scale×size modes, it compares THREE representations of the same intended world shape, ray-for-ray: (1) a
    /// built-in <c>*Collider2D</c> baked through the package into its Box2D-v3 world; (3) a
    /// <see cref="PhysicsShape2DAuthoring"/> baked through the package into the same world; (2) a live native
    /// <c>*Collider2D</c> driving Unity's own Box2D-v2 physics. The baked shapes (reps 1, 3) come from a temp
    /// SubScene authored by <c>Physics2DFixtures.RayScanParity</c> (the EditMode harness builds + loads it on
    /// demand) and run through the REAL bakers; the native shape (rep 2) is built live in this test.
    /// </summary>
    public sealed class AuthoringBakeRayScanEditMode : Physics2DEditModeHarness
    {
        const int Rays = 360;
        ...
        [Test]
        public void ThreeWayRayParity_AcrossKindsModes_BuiltinBake_CustomBake_Native()
        {
            LoadSubScene(Physics2DFixtures.RayScanParity, "RayScanParity");
            ...
            Assert.AreEqual(
                ...
                $"Expected {Expected} baked static bodies; got {shapeQuery.CalculateEntityCount()}."
            );

            // One creation step: PhysicsWorld2DSystem creates the Box2D bodies and packs userData, but the bodies
            // are static so they never integrate — leaving the query world populated for the scan.
            CreateBodies();
            Assert.AreEqual(
                Expected,
            ...
            for (var ki = 0; ki < 4; ki++)
            {
                for (var mi = 0; mi < 3; mi++)
                {
                    ...
                    UnityEngine.Object.DestroyImmediate(nativeGo);

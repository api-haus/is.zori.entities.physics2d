...
| Built-in 2D feature | Package mechanism | State | Exercising test | Worst error |
|---|---|---|---|---|
| custom `PhysicsBody2DAuthoring` / `PhysicsShape2DAuthoring` (the complete surface: interpolation, CCD, the mass-distribution override, free box/capsule orientation, the `PhysicsMaterial2D` material template + per-field override, the explicit filter bitset, the Collide/Sensor response, auto-fit, and the 2D-native editor) | direct runtime bake → same archetype | at-parity | `CustomAuthoringParityValidation`, `MaterialTemplateBakeGate`, `CustomAuthoring2DSampleBakeSmoke`, the Phase-A/C/D gates | **0.0** (bit-identical to the built-in baker, v3-vs-v3; the material-template shape converges bit-identical to a built-in collider carrying the same `sharedMaterial`). The complete field surface, the deliberate 2D negative space (no Cylinder / Plane / 3D-mesh / perspective handles / async previews / category-names SO), and the two known limitations (concave-decompose auto-fit, capsule-handle re-centring) are documented in [custom authoring](custom-authoring.md). |

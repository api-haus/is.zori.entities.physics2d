...
- `SetTransform(buffer, position, rotationRadians)`, `SetPosition(buffer, position)`, `SetRotation(buffer, rotationRadians)` — the INSTANTANEOUS pose set, a hard teleport (a direct write to `PhysicsBody.transform`, the native `b2Body_SetTransform`), distinct from the swept `MovePosition` / `MoveRotation`.
- `SkipInterpolation(buffer)` — suppress the next render-rate interpolation for the body, the 2D analogue of `CharacterInterpolation.SkipNextInterpolation()`.
...
### Teleport — `SetTransform` + `SkipInterpolation`

`SetTransform` is a hard, instantaneous pose set, the direct counterpart of writing `Rigidbody2D.position` / `rotation` (or, in 3D DOTS terms, `LocalTransform.Position` / `.Rotation`). It writes the body's `transform` directly — the native `b2Body_SetTransform` — with no `deltaTime` and no velocity, so unlike the swept `MovePosition` (`SetTransformTarget`) it carries no clamp: the world `maximumLinearSpeed` ceiling gates only the velocity-based swept move, never the instantaneous set, so a body reaches an arbitrarily far destination in one step regardless of distance. `SetPosition` and `SetRotation` set one axis and keep the body's current value on the other, mirroring how the swept `Move*` kinds leave the unchanged axis alone (`SetRotation` therefore lands the exact angle in one step, with none of the swept `MoveRotation`'s single-step undershoot). A `SetTransform` does NOT clear the body's velocity — for a respawn-style teleport that drops momentum, pair it with a `SetLinearVelocity` / `SetAngularVelocity` write.

`SkipInterpolation` suppresses the next render-rate interpolation for the body so the next frame draws the teleported pose with no smoothing streak from the old location — the 2D analogue of the 3D `CharacterInterpolation.SkipNextInterpolation()`. It resets the body's `PhysicsBody2DSmoothing` to `prev == cur == the body's live pose` with `hasPrev = 0` (the same shape a freshly-created body's smoothing is seeded with), which the render-rate `PhysicsBody2DSmoothingSystem` reads as "write the current pose, do not interpolate." It reads the body's pose at drain time, so it must be appended AFTER the `SetTransform` in the same frame (both drain in order before the step); it is a no-op on a body with no smoothing component (interpolation `None`), which has no streak to suppress. The canonical worked example is the controller-2d Platformer sample's `TeleporterSystem2D`, which on a character's trigger-enter enqueues `SetTransform(destination)` + `SkipInterpolation()`, mirroring the 3D sample's `TeleporterSystem`.
